    ...
    with mock.patch.dict(os.environ, {"LORE_CONFIG_DIR": str(tmp_path)}):
        ...
        cfg.write_text(json.dumps({
            "lore": {
                ...
                "search": {"embedding_provider": "ollama", "embedding_model": "nomic"},
                "git": {"provider": "gitlab"},
                "sync_interval": "1h",
            }
        }))
...
def test_get_project_config_with_hierarchy(tmp_path):
    ...
    cfg.write_text(json.dumps({
        "lore": {
            "hierarchy": [
                {"level": 1, "name": "team", "repo": "github.com/org/k", "branch": "team"},
                {"level": 2, "name": "product", "repo": "github.com/org/k", "branch": "product"},
                {"level": 3, "name": "company", "repo": "github.com/org/k", "branch": "org"},
            ]
        }
    }))
    pc = get_project_config(tmp_path)
...
def test_projects_array_readable(tmp_path):
    with mock.patch.dict(os.environ, {"LORE_CONFIG_DIR": str(tmp_path)}):
        ...
        cfg = tmp_path / "config.json"
        cfg.write_text(json.dumps({
            "lore": {"projects": ["/home/dev/proj-a", "/home/dev/proj-b"]}
        }))
        gc = get_global_config()
